...
- `integrations/hermes/` — Hermes log tailing, incident classification, correlator, and sinks.
...
Integration work usually spans config normalization, verification, integration-local clients/helpers, tools, docs, and tests. See [docs/adding-tools-and-integrations.md](docs/adding-tools-and-integrations.md) for the full file list, examples from the repo (Datadog, Grafana, Hermes), and the detailed definition of done (core completeness, tool wiring, docs/tests, `make verify-integrations`, final demo gate).
