...
def _emit(
    ...
    config: Any,
    *,
    exclude_integration_id: bool = True,
    **extra: Any,
) -> None:
    """Append an active env record for ``service`` from a validated ``config``."""
    payload = (
        config.model_dump(exclude={"integration_id"})
        if exclude_integration_id
        else config.model_dump()
    )
    integrations.append(_active_env_record(service, payload, **extra))
...
def _emit_guarded(
    ...
    *,
    ...
    """Build a config and emit its record, reporting (not raising) on failure.

    Mirrors the per-integration ``try/except _report_env_loader_failure`` guard so
    one malformed env value cannot abort discovery of the other integrations.
    """
    try:
        config = build()
        ...
        return
    _emit(integrations, service, config, exclude_integration_id=exclude_integration_id)


def _emit_instances(integrations: list[dict[str, Any]], env_name: str, service: str) -> bool:
    """Append a multi-instance record from ``env_name`` if set; return whether it did.

    Callers fall back to single-instance env vars only when this returns ``False``.
    """
    multi = _parse_instances_env(env_name, service)
    if multi is not None:
        integrations.append(multi)
        return True
    return False
...
def load_env_integrations() -> list[dict[str, Any]]:
    ...
    if not _emit_instances(integrations, "GRAFANA_INSTANCES", "grafana"):
        ...
        if grafana_endpoint and grafana_api_key:
            grafana_config = GrafanaIntegrationConfig.model_validate(
                {"endpoint": grafana_endpoint, "api_key": grafana_api_key}
            )
            integrations.append(
                _active_env_record(
                    "grafana",
                    {"endpoint": grafana_config.endpoint, "api_key": grafana_config.api_key},
                )
            )

    if not _emit_instances(integrations, "DD_INSTANCES", "datadog"):
        ...
        if datadog_api_key and datadog_app_key:
            _emit(
                integrations,
                "datadog",
                DatadogIntegrationConfig.model_validate(
                    {"api_key": datadog_api_key, "app_key": datadog_app_key, "site": datadog_site}
                ),
            )

    if not _emit_instances(integrations, "GROUNDCOVER_INSTANCES", "groundcover"):
        groundcover_api_key = (
            os.getenv("GROUNDCOVER_API_KEY", "").strip()
            or os.getenv("GROUNDCOVER_MCP_TOKEN", "").strip()
        )
        if groundcover_api_key:
            # Validates the MCP URL (HTTPS-or-loopback), which can raise on a bad
            # GROUNDCOVER_MCP_URL; guard so one malformed value cannot abort discovery.
            _emit_guarded(
                integrations,
                "groundcover",
                lambda: GroundcoverIntegrationConfig.model_validate(
                    {
    ...
    if not _emit_instances(integrations, "HONEYCOMB_INSTANCES", "honeycomb"):
    ...
    if not _emit_instances(integrations, "CORALOGIX_INSTANCES", "coralogix"):
    ...
    if not _emit_instances(integrations, "AWS_INSTANCES", "aws"):
        ...
        if aws_role_arn:
            aws_config = AWSIntegrationConfig.model_validate(
                {"role_arn": aws_role_arn, "external_id": aws_external_id, "region": aws_region}
            )
            integrations.append(
                _active_env_record(
                    "aws",
            ...
            build_github_mcp_config(
                {
                    ...
                    "toolsets": [p.strip() for p in github_toolsets.split(",") if p.strip()],
                }
            ),
        )
    ...
    if redis_config:
        _emit(integrations, "redis", redis_config)
    ...
    if not _emit_instances(integrations, "ARGOCD_INSTANCES", "argocd"):
        ...
        if argocd_base_url and (argocd_auth_token or (argocd_username and argocd_password)):
            _emit_guarded(
                integrations,
                "argocd",
                lambda: ArgoCDIntegrationConfig.model_validate(
                    {
    ...
    if helm_env_enabled:
        _emit_guarded(
            integrations,
            "helm",
            lambda: HelmIntegrationConfig.model_validate(
                {
    ...
    if vercel_api_token:
        _emit_guarded(
            integrations,
            "vercel",
            lambda: VercelConfig.model_validate(
                {"api_token": vercel_api_token, "team_id": os.getenv("VERCEL_TEAM_ID", "").strip()}
            ),
        )
    ...
    if opsgenie_api_key:
        _emit_guarded(
            integrations,
            "opsgenie",
            lambda: OpsGenieIntegrationConfig.model_validate(
                {
    ...
    if pagerduty_api_key:
        pagerduty_envs: dict[str, Any] = {"api_key": pagerduty_api_key}
        if _pd_base := os.getenv("PAGERDUTY_BASE_URL", "").strip():
            pagerduty_envs["base_url"] = _pd_base
        _emit_guarded(
            integrations,
            "pagerduty",
            lambda: PagerDutyIntegrationConfig.model_validate(pagerduty_envs),
        )
    ...
    if incident_io_api_key:
        _emit_guarded(
            integrations,
            "incident_io",
            lambda: IncidentIoIntegrationConfig.model_validate(
                {
    ...
    if jira_base_url and jira_email and jira_api_token:
        _emit_guarded(
            integrations,
            "jira",
            lambda: JiraIntegrationConfig.model_validate(
                {
    ...
    if discord_bot_token:
        _emit_guarded(
            integrations,
            "discord",
            lambda: DiscordBotConfig.model_validate(
                {
    ...
    if airflow_config is not None:
        _emit(integrations, "airflow", airflow_config, exclude_integration_id=False)
    ...
    if telegram_bot_token:
        _emit_guarded(
            integrations,
            "telegram",
            lambda: TelegramBotConfig.model_validate(
                {
    ...
    if smtp_host:
        _emit_guarded(
            integrations,
            "smtp",
            lambda: SMTPIntegrationConfig.model_validate(
                {
                    "host": smtp_host,
    ...
    # Shared Twilio account credentials — consumed by both the WhatsApp and SMS blocks.
    ...
    # Twilio SMS — independent of the legacy WhatsApp record; needs an SMS sender
    ...
    if (
        ...
        if twilio_config is not None:
            _emit(integrations, "twilio", twilio_config)
    ...
    if atlas_pub and atlas_priv and atlas_project:
        _emit_guarded(
            integrations,
            "mongodb_atlas",
            lambda: build_mongodb_atlas_config(
                {
    ...
    openclaw_mode = (
        os.getenv("OPENCLAW_MCP_MODE", "streamable-http").strip().lower() or "streamable-http"
    )
            ...
            openclaw_config = build_openclaw_config(
                {
                    ...
                    "args": [p for p in os.getenv("OPENCLAW_MCP_ARGS", "").strip().split() if p],
    ...
    posthog_mcp_mode = (
        os.getenv("POSTHOG_MCP_MODE", "streamable-http").strip().lower() or "streamable-http"
    )
            ...
            posthog_mcp_config = build_posthog_mcp_config(
                {
                    ...
                    "args": [p for p in os.getenv("POSTHOG_MCP_ARGS", "").strip().split() if p],
    ...
    sentry_mcp_mode = (
        os.getenv("SENTRY_MCP_MODE", "streamable-http").strip().lower() or "streamable-http"
    )
            ...
            sentry_mcp_config = build_sentry_mcp_config(
                {
                    ...
                    "args": [p for p in os.getenv("SENTRY_MCP_ARGS", "").strip().split() if p],
    ...
    if rds_config is not None and rds_config.is_configured:
        _emit(integrations, "rds", rds_config)
    ...
    if bs_endpoint and bs_username:
        _emit_guarded(
            integrations,
            "betterstack",
            lambda: build_betterstack_config(
                {
    ...
    if alertmanager_url:
        _emit_guarded(
            integrations,
            "alertmanager",
            lambda: AlertmanagerIntegrationConfig.model_validate(
                {
    ...
    if victoria_logs_url:
        _emit_guarded(
            integrations,
            "victoria_logs",
            lambda: VictoriaLogsIntegrationConfig.model_validate(
                {
    ...
    if not _emit_instances(integrations, "SPLUNK_INSTANCES", "splunk"):
        ...
        if splunk_url and splunk_token:
            _emit(
                integrations,
                "splunk",
                SplunkIntegrationConfig.model_validate(
                    {
                        "base_url": splunk_url,
                        "token": splunk_token,
    ...
    if supabase_url and supabase_service_key:
        try:
            ...
            integrations.append(_active_env_record("supabase", {"project_url": sb_config.url}))
        except Exception as exc:
    ...
    try:
        ...
        if signoz_config is not None and signoz_config.is_configured:
            _emit(integrations, "signoz", signoz_config)
    except Exception:
    ...
    try:
        ...
        if jenkins_config is not None and jenkins_config.is_configured:
            _emit(integrations, "jenkins", jenkins_config)
    except Exception:
    ...
    try:
        ...
        if tempo_config is not None and tempo_config.is_configured:
            _emit(integrations, "tempo", tempo_config)
    except Exception:
    ...
    if temporal_url and temporal_namespace:
        _emit_guarded(
            integrations,
            "temporal",
            lambda: TemporalConfig.model_validate(
                {
