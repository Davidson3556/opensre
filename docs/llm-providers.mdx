...
Requires the [GitHub Copilot CLI](https://docs.github.com/copilot/how-tos/use-copilot-agents/use-copilot-cli) (`npm i -g @github/copilot`). Copilot CLI has no scriptable `auth status` subcommand (only the interactive `/login`, `/user`, `/logout` slash commands and the `copilot login` device flow), so auth is detected via documented credential storage. **Probe order is CLI-first, env tokens last** (per PR #1533 review): (1) platform credential store — macOS Keychain via `security find-generic-password`, Linux libsecret via `secret-tool lookup`; silent metadata probe, the secret value is never read; (2) `$COPILOT_HOME/config.json` (default `~/.copilot/config.json`), parsed and validated as a non-empty JSON object; (3) `gh auth token` (cross-platform `gh` CLI fallback — Copilot accepts gh's stored token); (4) `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` / `GITHUB_TOKEN` env (documented automation bypass). On Windows there is no reliable silent probe for Credential Manager, so detection reports `logged_in=None` and the runner verifies at invoke time — `explain_failure` then surfaces the auth hint if stderr matches an auth error pattern. If `COPILOT_MODEL` is unset, OpenSRE omits the `--model` flag and the CLI uses its configured default. Invocations run as `copilot -p PROMPT --no-color --no-ask-user --silent` so they never block on user input.
