...
| Command | What it does |
| --- | --- |
| `/account` | Sign in to OpenSRE with GitHub, inspect the local account, or sign out |
...
| `toolcall set &lt;model&gt;` | Set the tool-call model for the active provider |
...
Sign in to a personal OpenSRE account with GitHub, inspect the local login, or sign out. Same as `opensre account`. Interactive login opens a browser and prompts on the real TTY. If `OPENSRE_ACCOUNT_TOKEN` is set in the environment, it overrides the token saved by login until you unset it.
...
Tail Hermes logs and escalate classified incidents. See [Hermes](/hermes).
