...
These menus stay open. A read-only pick such as `/model show` prints its output
and returns you to the menu, so leave with **done** or Esc when you are finished.
A pick that changes something closes the menu itself.
...
Prints `No selection menu is pending.` when the agent has not asked anything. Outside a TTY it lists the options as text for you to answer in a reply instead of opening a picker.
